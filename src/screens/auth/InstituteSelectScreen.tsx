...
import { getInstituteLogoByName } from '../../config/instituteLogo';
...
const InstituteSelectScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // ─── Data Normalisation ────────────────────────────────────────────────────
  // ✅ UPDATED: Now uses getInstituteLogoByName to get logos
  const normalized: NormalizedInstitute[] = (
    institutes as NormalizedInstitute[]
  ).map(i => ({
    ...i,
    name: i.name,
    logo: getInstituteLogoByName(i.name),
    city: i.city || '',
    state: i.state || '',
    type: i.type || 'School',
  }));
  ...
  const handleSelect = async (
    ...
    const inst =
      institute as NormalizedInstitute;
      ...
      <FlatList<NormalizedInstitute>
        ...
        ListHeaderComponent={
          <>
            ...
            {filtered.length === 0 && (
              <Text testID="emptyInstituteMessage" style={[styles.emptyText, { color: colors.textMuted }]}>
