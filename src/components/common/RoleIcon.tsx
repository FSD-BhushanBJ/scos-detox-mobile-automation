...
import { getRoleIcon } from '../../config/roleIcons';
...
const RoleIcon: React.FC<RoleIconProps> = ({
  ...
  const CustomIcon = getRoleIcon(roleName);
  if (CustomIcon) {
    return <CustomIcon color={color} size={size} />;
  }
  ...
  return (
    <MaterialIcons name={normalizedName as any} size={size} color={color} />
  );
};
